...
export function Theme({
  ...
  if (variant === "button") {
    ...
    const safeTheme: ThemeType =
      isTheme(theme) && themes.includes(theme as ThemeType) ? (theme as ThemeType) : "light"
              ...
              >
                {isSelected && (
                  <motion.div
                    ...
                    className="absolute inset-0 rounded-md bg-[var(--color-card)] dark:bg-slate-800 bg-white shadow-sm"
                    transition={{ type: "spring", stiffness: 500, damping: 30 }}
                  />
                )}
...
export type ThemeType = "light" | "dark" | "system" | "sunset" | "ocean" | "forest"
// Re-exporting as Theme inside to match user's demo
